...
export default (fn, setup, duration) => {
  if (typeof fn !== "function") {
    throw new TypeError("Expected benchmark subject to be a function");
  }
  if (setup == null) {
    setup = fn.length
      ? (cb) => {
  ...
  if (typeof setup !== "function") {
    throw new TypeError("Expected benchmark setup to be a function");
  }
  if (!Number.isInteger(duration)) {
    throw new TypeError("Expected benchmark duration to be an integer");
  }
  if (duration <= 0) {
    throw new TypeError("Expected benchmark duration to be positive");
  }
