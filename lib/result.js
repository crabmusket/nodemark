...
const getMgnError = (arr, mean) =>
  Math.sqrt(arr.reduce(fSumSqDiff(mean), 0)) / arr.length * 2;
...
export default class BenchmarkResult {
  ...
  toString(format = "hz") {
    if (!unitMap[format]) {
      throw new TypeError(`Unrecognized toString format: ${format}`);
    }
...
const round = (n, e) => {
  if (e >>> 0 !== e || e > 6) {
    throw new TypeError(
      "Rounding precision must be an integer between 0 and 6",
    );
  }
