const async = (fn, setup, duration, callback) => {
  ...
  const setupDone = (id) =>
    (err) => {
      if (id !== currentId) {
        return callback(
          new TypeError("The benchmark setup callback was invoked twice"),
        );
      }
  ...
  const noopDone = (fake1) =>
    (fake2) => {
      const t1 = performance.now();
      if (fake1 !== currentId) {return callback(
          new TypeError("THIS SHOULD NEVER HAPPEN"),
        );}
  ...
  const subjectDone = (id) =>
    (err) => {
      const t3 = performance.now();
      if (id !== currentId) {
        return callback(
          new TypeError("The benchmark subject callback was invoked twice"),
        );
      }
      if (err) return callback(err);
      samples.push((t3 - t2) * 1e6 - noise);
      currentId += 1;
      if (Date.now() < duration) queueMicrotask(runIteration);
      else callback(null, samples);
    };

  const runIteration = () => {
    setup(setupDone(currentId));
  };
  queueMicrotask(runIteration);
};

export default (fn, setup, duration) => {
  let callback;
  const promise = new Promise((resolve, reject) => {
    callback = (err, samples) => err ? reject(err) : resolve(samples);
  });
